...
/**
 * Servlet implementation class UrunDetayServlet
 */
@WebServlet("/UrunDetayServlet")
public class UrunDetayServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	private static Logger logger = LoggerFactory.getLogger(UrunDetayServlet.class);
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UrunDetayServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			json.addProperty("yorum", list.get(i).getUrunYorum());
			json.addProperty("detay", list.get(i).getUrunDetay());
			json.addProperty("begeni", list.get(i).getUrunBegeni());
			json.addProperty("categori", list.get(i).getKategori().getCat_name());
			data_json.add(json);
		}
